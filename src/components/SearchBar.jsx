...
export default function SearchBar({
  ...
  const handleKeyPress = (e) => {
    if (e.key === "Enter") {
      const match = pokemonList.find(p =>
        p.label.toLowerCase() === search.toLowerCase()
      );
      const original = match ? match.id : search;
      onSearch(original);
      setSuggestions([]);
    }
  };
  ...
  const handleChange = (e) => {
    ...
    const filtered = pokemonList
      .filter(p => p.label.toLowerCase().includes(value))
      .slice(0,  50);
  ...
  const handleRandomSearch = () => {
    ...
    onSearch(random.id).then(() => {
      setSearch(random.label);
      setSuggestions([]);
    });
  };
      ...
      <div ref={wrapperRef} className="relative w-full max-w-2xl">
        <div className="flex w-full">
          ...
          <input
            ...
            className="w-full px-4 py-2 md:px-6 md:py-3 text-[#01302c] bg-white focus:outline-none"
          ...
          <button
            ...
            className="px-4 py-2 md:px-6 md:py-3 text-[#01302c] font-bold bg-[#51ae5e] cursor-pointer rounded-r-full"
          >
            BUSCAR
          </button>
        </div>
          ...
          <ul className="absolute left-0 right-0 mt-1 bg-white text-black border border-gray-300 z-20 max-h-48 overflow-y-auto shadow-xl rounded-[28px] custom-scrollbar">
            {suggestions.map(({ id, label }) => (
              <li
                ...
                onClick={() => {
                  setSearch(label); // mostramos el traducido
                  setSuggestions([]);
                  onSearch(id); // buscamos con el nombre en inglés
                }}
              >
                {label}
              </li>
